...
export default function TeacherDashboard() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--bg-base)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
      {/* Orb */}
      <div aria-hidden style={{ position: "fixed", top: "10%", right: "10%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(37,99,235,0.2) 0%, transparent 70%)", filter: "blur(60px)", pointerEvents: "none" }} />
      ...
      <div className="animate-fade-up" style={{ textAlign: "center", maxWidth: 500 }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: "2rem" }}>
          <div style={{ width: 36, height: 36, borderRadius: 9, background: "linear-gradient(135deg, var(--teacher) 0%, var(--accent) 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ padding: "2.5rem", background: "var(--bg-card)", border: "1.5px solid var(--border)", borderRadius: "var(--radius-xl)", marginBottom: "1.5rem" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: "var(--teacher-dim)", border: "2px solid var(--teacher)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.5rem", fontSize: "1.5rem", color: "var(--teacher)", fontWeight: 700, fontFamily: "Space Grotesk, sans-serif" }}>
            {user.name.charAt(0).toUpperCase()}
          </div>
          <p style={{ fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--teacher)", marginBottom: "0.5rem" }}>Teacher Portal</p>
          ...
          <span style={{ fontSize: "0.75rem", background: "var(--teacher-dim)", color: "var(--teacher)", border: "1px solid var(--teacher)", borderRadius: 999, padding: "2px 10px" }}>Teacher</span>
          ...
          <div style={{ marginTop: "2rem", padding: "1.25rem", background: "var(--bg-surface)", borderRadius: "var(--radius-md)", border: "1px dashed var(--border)" }}>
            <p style={{ color: "var(--text-muted)", fontSize: "0.9rem", lineHeight: 1.6 }}>
              🚀 Your dashboard is being built. Test creation, student management, and analytics coming soon.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "1rem", justifyContent: "center" }}>
          ...
          <button onClick={handleLogout} style={{ padding: "10px 20px", borderRadius: "var(--radius-md)", background: "var(--teacher)", border: "none", color: "white", fontSize: "0.9rem", cursor: "pointer", fontFamily: "inherit" }}>Sign out</button>
        </div>
      </div>
    </main>
  );
